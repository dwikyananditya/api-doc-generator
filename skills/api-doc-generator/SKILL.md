...
2. **Choose scope.** One target always produces exactly one JSON and one DOCX, whether it is a single file, one module, or a folder containing many modules and subfolders. Document every endpoint found anywhere under the target (recursively) unless the user named specific ones; keep endpoints from the same module/subfolder next to each other in the `endpoints` array; do not ask, and never split the output into one file per endpoint. Each endpoint is one item in the `endpoints` array. When the same handler is exposed under several routes (e.g. `/capaian/...` and `/web/capaian/...`), write one item and list the other routes in `endpoint.aliases`.
...
5. **Analyze the source — inside `PROJECT_ROOT` only.** Start at the route handler and follow the direct imports it needs: service, DTO, repository/entity, HTTP client, and the guards/pipes/interceptors applied to it. Skip `node_modules`, build output, generated code, and unrelated modules.
   ...
   - For several endpoints in one module, trace the shared pieces (guards, interceptors, error types, HTTP client) once and reuse the findings for every endpoint in the document.
